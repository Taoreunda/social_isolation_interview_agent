...
모든 면담은 같은 안내로 시작한다. 모델이 쓰지 않고 `backend/interview/welcome.py`에 고정해 두며, 말풍선 두 개가 첫 메시지들로 저장된 뒤 첫 질문(A1)이 이어진다 — 질문까지 말풍선 세 개. 화면은 새로 시작한 면담에서만 말풍선을 하나씩 띄우고, 이어서 들어온 면담은 한 번에 보여 준다.

1. 안녕하세요, 반갑습니다. 저는 이 면담을 진행하는 AI입니다. 최근 한 달 동안 어떻게 지내셨는지 열 가지 남짓 여쭤볼게요. 10분 안팎이면 끝납니다.
2. 정답은 없습니다. 짧게 답하셔도 되고, 잘 모르겠으면 모르겠다고 하셔도 괜찮아요. 중간에 멈췄다가 나중에 이어서 하셔도 됩니다.
3. (첫 질문)

"10분 안팎"은 추정이다. 파일럿에서 실제 소요 시간을 재서 맞춘다.
...
|---|---|---|---|
| 예/아니요 | A1 | 예 · 아니요 | — |
| 횟수 | A2 | 주 0회 · 1회 · 2회 · 3회 · 4회 이상 | 경계가 4회이므로 "4회 이상"은 하나로 묶어도 판정에 영향 없음 |
