...
모든 면담은 같은 안내로 시작한다. 모델이 쓰지 않고 `backend/interview/welcome.py`에 고정해 두며, 말풍선 하나씩 첫 메시지들로 저장된 뒤 첫 질문(A1)이 이어진다. 화면은 새로 시작한 면담에서만 말풍선을 하나씩 띄우고, 이어서 들어온 면담은 한 번에 보여 준다.

1. 안녕하세요, 반갑습니다. 저는 이 면담을 진행하는 AI입니다.
2. 최근 한 달 동안 어떻게 지내셨는지 열 가지 남짓 여쭤볼 거예요. 10분 안팎이면 끝납니다.
3. 정답은 없습니다. 짧게 답하셔도 되고, 잘 모르겠으면 모르겠다고 하셔도 괜찮아요. 중간에 멈췄다가 나중에 이어서 하셔도 됩니다.
4. 그럼 첫 질문 드릴게요.
...
참여자가 말풍선 아래 칩을 눌러 답할 수 있게 하려면, 칩은 **판정 경계(cut-off)를 가르고 코드값을 그대로 낼 수 있게** 잡는다. 유형별로 한 세트를 쓰면 문항이 바뀌어도 같은 칩을 재사용할 수 있다.
