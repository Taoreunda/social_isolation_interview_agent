...
입력창 바로 위 왼쪽, 질문 아래에 지금 묻는 문항의 추천 답변이 뜬다. 말풍선과 같은 모양에 테두리만 있는 형태다. 보기가 있을 때 입력창에는 "위에서 고르거나 직접 입력하세요"라는 안내가 들어간다. 대화와 보기·입력창 사이에 구분선은 두지 않는다. 목록은 `backend/interview/suggestions.py`에 있고 참가자 응답의 `suggestedReplies`로 내려간다.
