"""The judging criteria handed to the interviewer, word for word.

These strings are part of the research instrument. They moved out of the legacy
prompt templates unchanged; this test keeps any later edit a deliberate one.
"""
...
def test_every_question_carries_its_criteria_unchanged() -> None:
    scorecard = Scorecard()

    assert list(EXPECTED) == scorecard.question_order
    for question_id, expected in EXPECTED.items():
        assert scorecard.get_criteria_text(question_id) == expected.rstrip("\n"), question_id
